...
namespace EverydayCallouts.Logging
{
    public static class Initializing
    {
        ...
        private static void PrintDependencyInfo()
        {
            ...
            PrintDependency("CalloutInterfaceAPI.dll", "1.0.3.0");
            PrintDependency("CommonDataFramework.dll");
            PrintDependency("LSPD First Response.dll");
        ...
        private static void PrintDependency(string fileName, string expectedVersion = null)
        {
            ...
            if (!File.Exists(path))
            {
                Game.Console.Print($"EverydayCallouts: [Dependencies] {fileName,-22} : MISSING");
                return;
            }
